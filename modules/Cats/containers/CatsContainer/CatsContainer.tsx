...
const CatsContainer = () => {
  ...
  const loaderContainer = typeof document !== 'undefined' ? document.getElementById('root') : null;
    ...
    <div className={styles.cats_container}>
      ...
      <div className={styles.imagees_content}>
        {catsState.map((item, index, array) =>{
          return (
            <Link href={`cat/${item.id}`} className={styles.image_item} key={index} ref={index === array.length - 1 ? lastItemRef : null}>
              <img width={200} height={200} src={item.url} alt="cat image"/>
              <div className="flex flex-col">
                <p className="text-md"><span className="font-semibold">Breed Name: </span>{item.breeds?.[0]?.name ?? '-'}</p>
                <p className="text-md"><span className="font-semibold">Width: </span>{item.width}px</p>
                <p className="text-md"><span className="font-semibold">Height: </span>{item.height}px</p>
              </div>
            </Link>
          )
        })}
